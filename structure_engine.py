...
# ---------------------------------------------------------------------------
# Phase status — a display-agnostic read of build_market_structure()'s
# result, used identically by views/market_structure.py (both its "Current
# Phase" panel and its "Advanced Structure Details" expander, which
# previously computed this same thing twice with slightly different
# phrasing) and by the API's Market Structure endpoint. Pure function over
# an already-computed `result` dict — no new analysis, just naming the
# same state-machine outcome three consistent ways.
# ---------------------------------------------------------------------------
def phase_status(result: dict) -> dict:
    state = result["state"]
    break_info = result.get("break")

    indication_confirmed = bool(break_info and break_info.get("broken"))
    correction_active = "pullback" in state or "correction_in_progress" in state
    continuation_active = "continuation" in state
    continuation_confirmed = "continuation_confirmed" in state
    continuation_developing = "continuation_developing" in state

    return {
        "indication_status": "Confirmed" if indication_confirmed else "Waiting",
        "correction_status": "In Progress" if correction_active else ("Passed" if continuation_active else "—"),
        "continuation_status": "Confirmed" if continuation_confirmed else ("Developing" if continuation_developing else "Waiting"),
...
def build_structure_payload(ticker: str, sensitivity: dict = None) -> dict:
    """API-shaped view of build_market_structure()'s result: per-timeframe
    swings/bias/levels (raw OHLC history dropped — not JSON-safe and not
    needed outside the Streamlit chart), buyer/seller levels collected
    across all four timeframes, and the same break/pullback/continuation
    read phase_status() already gives the Streamlit page. Pure reshaping of
    an already-computed result — no analysis happens here.
    """
    result = build_market_structure(ticker, sensitivity=sensitivity)
    phases = phase_status(result)

    timeframes = {}
    seller_levels = []
    buyer_levels = []
    for label, bundle in result["timeframes"].items():
        key = label.lower()
        if bundle is None:
            timeframes[key] = None
            continue
        ...
        timeframes[key] = {
            ...
            "levels": levels,
            "candle_count": bundle["candle_count"],
            "last_time": bundle["last_time"],
        }
        if levels.get("seller_level"):
            seller_levels.append({"timeframe": label, **levels["seller_level"]})
        if levels.get("buyer_level"):
            buyer_levels.append({"timeframe": label, **levels["buyer_level"]})

    return {
        "symbol": ticker,
        "direction": result["direction"],
        "structure_status": result["state_label"],
        ...
        "break": result.get("break"),
        "break_status": phases["indication_status"],
        "pullback_status": phases["correction_status"],
        "continuation_status": phases["continuation_status"],
        "advanced": {
            "indication": phases["indication_status"],
            "correction": phases["correction_status"],
