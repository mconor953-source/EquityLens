...
from structure_engine import build_market_structure, DEFAULT_SENSITIVITY, REQUIRE_CANDLE_CLOSE
...
with side_col:
    ...
    with st.container(border=True):
        ...
        indication_done = bool(break_info and break_info.get("broken"))
        correction_active = "pullback" in result["state"] or "correction_in_progress" in result["state"]
        continuation_active = "continuation" in result["state"]

        for phase_name, status in [
            ("Breakout / Breakdown", "Confirmed" if indication_done else "Waiting"),
            ("Pullback / Correction", "In Progress" if correction_active else ("Passed" if continuation_active else "—")),
            ("Continuation", "Confirmed" if "confirmed" in result["state"] and continuation_active else ("Developing" if "developing" in result["state"] else "Waiting")),
        ]:
            render_html(
...
with st.expander("Advanced Structure Details"):
    ...
    indication_status = "Confirmed" if (break_info and break_info.get("broken")) else "Waiting"
    correction_status = "In Progress" if ("pullback" in result["state"] or "correction_in_progress" in result["state"]) else ("Passed" if "continuation" in result["state"] else "—")
    continuation_status = "Confirmed" if "continuation_confirmed" in result["state"] else ("Developing" if "continuation_developing" in result["state"] else "Waiting")
    adv1.metric("Indication", indication_status)
    adv2.metric("Correction", correction_status)
    adv3.metric("Continuation", continuation_status)
