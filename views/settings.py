...
— out of scope for the navy/white design this app is built on).
"""
...
page_header("Settings", subtitle="Preferences for how EquityLens opens — saved to disk, so they persist across restarts.")
...
with col1:
    ...
    with st.container(border=True):
        st.caption("Market Research opens to this asset.")

        classes = list(PREDEFINED_ASSETS.keys())
        saved_class = settings.get("default_asset_class", "US Stocks")
...
if st.button("Save Preferences", type="primary"):
    ...
    st.success("Saved. Takes effect the next time you open Market Research.")
